...
public class SquirrelControl extends Activity {
    ...
    public void onCreate(Bundle savedInstanceState) {
		...
		actionRunnable[2] = new Runnable() {
			...
			public void run() {
				if (cameraStarted) {
					cameraStarted = false;
					sharedPreferences.edit().putBoolean(CAMERA_ACTIVITY, false).commit();
					cameraRestart = -1;
					...
					cameraStarted = true;
					sharedPreferences.edit().putBoolean(CAMERA_ACTIVITY, true).commit();
		...
		try {
			...
			new AlertDialog.Builder(this).setTitle("SquirrelSMS")
					.setMessage("The package uk.ac.cam.cusf.squirrelsms was not found!")
		...
		//confirm[2].setEnabled(false); // We aren't using SquirrelCamera this time
		...
		cameraRunnable = new Runnable() {
			...
			public void run() {
				if (cameraRestart < 0) {
					...
					status.setText("Restarting Squirrel Camera in "+cameraRestart+" seconds");
					cameraRestart--;
		...
		enabledServices.put(LOG_SERVICE, Monitoring.isServiceRunning(LOG_SERVICE, context) 
				|| sharedPreferences.getBoolean(LOG_SERVICE, false));
		enabledServices.put(RADIO_SERVICE, Monitoring.isServiceRunning(RADIO_SERVICE, context) 
				|| sharedPreferences.getBoolean(RADIO_SERVICE, false));
		action[0].setText(enabledServices.get(LOG_SERVICE) == Boolean.TRUE ? "Stop" : "Start");
		action[1].setText(enabledServices.get(RADIO_SERVICE) == Boolean.TRUE ? "Stop" : "Start");
		...
		serviceChecker = new Runnable() {
			@Override
			public void run() {
				if (!Monitoring.isServiceRunning(MonitorService.class.getName(), context)) {
    ...
    private Runnable actionRunnable(final int i, final String service, final String intentAction) {
    	return new Runnable() {
			@Override
			public void run() {
				if (enabledServices.get(service) == Boolean.TRUE) {
					enabledServices.put(service, false);
					sharedPreferences.edit().putBoolean(service, false).commit();
